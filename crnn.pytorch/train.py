...
parser.add_argument('--trainRoot',  help='path to dataset',default='/HomeWork/crnn.pytorch/lmdb/train_lmdb')
parser.add_argument('--valRoot',  help='path to dataset',default='/HomeWork/crnn.pytorch/lmdb/val_lmdb')
...
def val(net, dataset, criterion, max_iter=100):
    ...
    for i in range(max_iter):
        ...
        vallist = []
        for i in cpu_texts:
                vallist.append(i.decode('utf-8', 'strict'))
        for pred, target in zip(sim_preds, vallist):
            if pred == target:
                n_correct += 1

    raw_preds = converter.decode(preds.data, preds_size.data, raw=True)[:opt.n_test_disp]
    for raw_pred, pred, gt in zip(raw_preds, sim_preds, vallist):
...
for epoch in range(opt.nepoch):
    ...
    # do checkpointing 10次epoch存储一次模型
    #if epoch % opt.saveInterval == 0:
    #每25%进度存储一次模型
    if epoch % int(opt.nepoch/4) == 0:
